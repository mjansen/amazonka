...
data Limits = Limits'{_lInstanceLimits ::
...
limits :: Limits
limits = Limits' { _lInstanceLimits   = Nothing
...
lAdditionalLimits =
  lens _lAdditionalLimits (\s a -> s { _lAdditionalLimits = a })
    . _Default
    . _Coerce
...
lStorageTypes =
  lens _lStorageTypes (\s a -> s { _lStorageTypes = a }) . _Default . _Coerce
