...
data LogPublishingOption = LogPublishingOption'{_lpoEnabled
                                                :: !(Maybe Bool),
...
lpoCloudWatchLogsLogGroupARN = lens
  _lpoCloudWatchLogsLogGroupARN
  (\s a -> s { _lpoCloudWatchLogsLogGroupARN = a })
...
instance ToJSON LogPublishingOption where
  toJSON LogPublishingOption' {..} = object
    (catMaybes
      [ ("Enabled" .=) <$> _lpoEnabled
      , ("CloudWatchLogsLogGroupArn" .=) <$> _lpoCloudWatchLogsLogGroupARN
      ]
    )
