...
data VPCOptions = VPCOptions'{_voSecurityGroupIds ::
                              !(Maybe [Text]),
...
vpcOptions =
  VPCOptions' { _voSecurityGroupIds = Nothing, _voSubnetIds = Nothing }

-- | Specifies the security groups for VPC endpoint.
voSecurityGroupIds :: Lens' VPCOptions [Text]
voSecurityGroupIds =
  lens _voSecurityGroupIds (\s a -> s { _voSecurityGroupIds = a })
    . _Default
    . _Coerce
...
voSubnetIds =
  lens _voSubnetIds (\s a -> s { _voSubnetIds = a }) . _Default . _Coerce
