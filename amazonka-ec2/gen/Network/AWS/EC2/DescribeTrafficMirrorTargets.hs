...
instance AWSPager DescribeTrafficMirrorTargets where
        page rq rs
          ...
          | otherwise =
            Just $ rq & dtmtNextToken .~ rs ^. dtmtrsNextToken

instance AWSRequest DescribeTrafficMirrorTargets
         where
...
instance Hashable DescribeTrafficMirrorTargets where

instance NFData DescribeTrafficMirrorTargets where
