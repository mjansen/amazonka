...
instance AWSPager DescribeLocalGatewayRouteTables
         where
        page rq rs
          | stop (rs ^. dlgrtrsNextToken) = Nothing
          | stop (rs ^. dlgrtrsLocalGatewayRouteTables) =
            Nothing
          | otherwise =
            Just $ rq & dlgrtNextToken .~ rs ^. dlgrtrsNextToken

instance AWSRequest DescribeLocalGatewayRouteTables
         where
...
instance NFData DescribeLocalGatewayRouteTables where

instance ToHeaders DescribeLocalGatewayRouteTables
         where
...
instance ToQuery DescribeLocalGatewayRouteTables
         where
