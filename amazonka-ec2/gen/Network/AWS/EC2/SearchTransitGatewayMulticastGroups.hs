...
instance AWSPager SearchTransitGatewayMulticastGroups
         where
        page rq rs
          | stop (rs ^. stgmgrsNextToken) = Nothing
          | stop (rs ^. stgmgrsMulticastGroups) = Nothing
          | otherwise =
            Just $ rq & stgmgNextToken .~ rs ^. stgmgrsNextToken
...
instance ToPath SearchTransitGatewayMulticastGroups
         where
        toPath = const "/"

instance ToQuery SearchTransitGatewayMulticastGroups
         where
